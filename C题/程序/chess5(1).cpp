...
bool JudgeFourOrFive(int colour) {
	...
	{
		cout<<"x:"<<PointSet1[i].x<<endl;
		cout<<"y:"<<PointSet1[i].y<<endl;
	}
	for(int i=0;i<PointSet2.size();i++)
	{
		cout<<"x1:"<<PointSet2[i].x1<<endl;
		cout<<"y1:"<<PointSet2[i].y1<<endl;
		cout<<"x2:"<<PointSet2[i].x2<<endl;
		cout<<"y2:"<<PointSet2[i].y2<<endl;
	}
	cout<<"$$$$$$$$$$$$$$$$$$$$$$$"<<endl;
...
//			cout<<typeid((*Point1).x).name()<<endl;
...
//		}
//	}
	int q=PointSet1.size();
	int sum=0;
	for(int i=0;i<q-1;i++)
	{
		for(int j=i+1;j<q;j++)
		{
			if(PointSet1[i].x==PointSet1[j].x&&PointSet1[i].y==PointSet1[j].y)
			{
				sum++;
				for(int k=j;k<q-1;k++)
				{
					PointSet1[k]=PointSet1[k+1];
				}
			}
		}
	}
	...
	for(int i=0;i<q-1;i++)
	{
		for(int j=i+1;j<q;j++)
		{
			bool Q1=PointSet2[i].x1==PointSet2[j].x1&&PointSet2[i].y1==PointSet2[j].y1;
			bool Q2=PointSet2[i].x2==PointSet2[j].x2&&PointSet2[i].y2==PointSet2[j].y2;
			bool Q3=PointSet2[i].x1==PointSet2[j].x2&&PointSet2[i].y1==PointSet2[j].y2;
			bool Q4=PointSet2[i].x2==PointSet2[j].x1&&PointSet2[i].y2==PointSet2[j].y1;
			if((Q1&&Q2)||(Q3&&Q4))
			{
				sum++;
				for(int k=j;k<q-1;k++)
				{
					PointSet2[k].x1=PointSet2[k+1].x1;
					PointSet2[k].y1=PointSet2[k+1].y1;
					PointSet2[k].x2=PointSet2[k+1].x2;
					PointSet2[k].y2=PointSet2[k+1].y2;
				}
				q--;
			}
		}
...
//			Board[step.second.x][step.second.y] = computerSide;
			...
			if(JudgeFourOrFive(computerSide)) {
				...
				cout<<"3"<<endl;
				while (Board[step.first.x][step.first.y] != 2||step.first.x<0||step.first.x>=19||step.first.y<0||step.first.y>=19) {
					step.first.x = rand() % 19;
					step.first.y = rand() % 19;
					cout<<"while1"<<endl;
				}
				while (Board[step.second.x][step.second.y] != 2||step.second.x<0||step.second.x>=19||step.second.y<0||step.second.y>=19) {
					step.second.x = rand() % 19;
					step.second.y = rand() % 19;
					cout<<"while2"<<endl;
				}
				while((step.first.x==step.second.x&&step.first.y==step.second.y)||(Board[step.second.x][step.second.y] != 2||step.second.x<0||step.second.x>=19||step.second.y<0||step.second.y>=19)) {
					step.second.x = rand() % 19;
					step.second.y = rand() % 19;
					cout<<"while3"<<endl;
				}
...
//					step.second.x = rand() % 19;
//					step.second.y = rand() % 19;
					if(P2S==1&&SS<PointSet2.size())
					{
						step.second.x=PointSet2[SS].x1;
						step.second.y=PointSet2[SS].y1;
						SS++;
					}
					cout<<"while7"<<endl;
					if(P2S==0||SS==PointSet2.size())
					break;
				}
				SS=0;
				while((step.first.x==step.second.x&&step.first.y==step.second.y)||(Board[step.second.x][step.second.y] != 2||step.second.x<0||step.second.x>=19||step.second.y<0||step.second.y>=19)) {
...
//					step.second.y = rand() % 19;
					if(P2S==1&&SS<PointSet2.size())
					{
						step.second.x=PointSet2[SS].x2;
						step.second.y=PointSet2[SS].y2;
						SS++;
					}
					cout<<"while6"<<endl;
					if(P2S==0||SS==PointSet2.size())
				...
				Board[step.first.x][step.first.y] = computerSide;
				Board[step.second.x][step.second.y] = computerSide;
			} else if(JudgeThree(Board,computerSide)) {
				cout<<"10"<<endl;
				//填充落子代码
				int dv[4]= {0,1,1,-1};
				...
				for(int i=-1; i<19; i++) {
					...
					if(Board[x][y]==EMPTY&&x!=step.first.x&&y!=step.first.y) {
						step.second.x=x;
						step.second.y=y;
						bj=1;
						break;
				...
				Board[step.first.x][step.first.y] = computerSide;
				Board[step.second.x][step.second.y] = computerSide;
			} else if(JudgeThree(Board,1-computerSide)) {
				cout<<"11"<<endl;
				//填充落子代码
				int dv[4]= {0,1,1,-1};
				int dw[4]= {1,0,1,1}; //代表方向，分别为下，右，右下，左
					...
					for(int j=0; j<19; j++) {
						if(Board[i][j]!=EMPTY) {
							for(int r=0; r<8; r++) {
								int x=i+dv[r];
								int y=j+dw[r];
								if(x>=0&&y>=0&&x<19&&y<19&&Board[x][y]==EMPTY&&x!=step.first.x&&y!=step.first.y) {
									step.second.x=x;
									step.second.y=y;
									bj++;
								}
				...
				if(bj==0) {
					...
					for(int i=0; i<19; i++) {
						for(int j=0; j<19; j++) {
							if(Board[i][j]==2&&(i!=step.first.x||y!=step.first.y)) {
								x=i;
								y=j;
								k=1;
								break;
			...
			/*while(step.first.x==step.second.x&&step.first.y==step.second.y) {
				step.second.x = rand() % 19;
				step.second.y = rand() % 19;
			}
			while (Board[step.first.x][step.first.y] != 2||step.first.x<0||step.first.x>=19||step.first.y<0||step.first.y>=19) {
			...
			/*while(step.second.x==step.first.x&&step.second.y==step.second.y)
			{
